...
class AboutClassMethods < EdgeCase::Koan
  ...
  def test_objects_have_methods
    fido = Dog.new
    assert fido.methods.size > 0          # actually has 65 methods ?!? that many inherited? 
  end
  ...
  def test_classes_and_instances_do_not_share_instance_variables                      # WTF ?!?!?!?!
    fido = Dog.new                                                                    # Class-level variables (~static)
    fido.name = "Fido"                                                                # defined with Class.label
  ...
  class Person
    class << self
      protected
      def find(id)
        # this approach can be used to make class/static method protected
      end
    end
  end
